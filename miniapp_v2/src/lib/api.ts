...
export async function setDriverOnline(input: { is_online: boolean; country_code?: string | null; city_id?: number | null }): Promise<DriverOnlineState> { const data = await request<any>('/driver/online', { method: 'POST', body: JSON.stringify({ is_online: input.is_online }) }); return { is_online: Boolean(data?.is_online), is_busy: Boolean(data?.is_busy), country_code: input.country_code || data?.country || null, city_id: input.city_id ?? null, lat: data?.lat ?? null, lng: data?.lng ?? null }; }
...
export async function listMyCityOrders(): Promise<CityOrder[]> { const data = await request<any>('/city/orders/my'); return unwrapItems<BackendCityOrder>(data).map((item) => normalizeCityOrder(item)); }
...
export async function cancelCityOrder(orderId: number, reason?: string): Promise<CityOrder> { const data = await request<any>(`/city/orders/${orderId}/status`, { method: 'POST', body: JSON.stringify({ status: 'cancelled', reason: reason || null }) }); return normalizeCityOrder(unwrapItem(data)); }
export async function listAvailableCityOrders(): Promise<CityOrder[]> { const data = await request<any>('/city/orders/available'); return unwrapItems<BackendCityOrder>(data).map((item) => normalizeCityOrder(item)); }
export async function acceptCityOrder(orderId: number): Promise<CityTrip> { const data = await request<any>(`/city/orders/${orderId}/accept`, { method: 'POST' }); return normalizeCityTrip(unwrapItem(data)); }

async function getCurrentTrip() { const data = await requestFirst<any>(['/trips/current', '/trip/current', '/current-trip']); return data?.item || data || null; }
export async function getCurrentCityTrip(): Promise<CityTrip | null> { try { const item = await getCurrentTrip(); if (!item) return null; const type = String(item.trip_type || item.source_type || 'city_trip'); return type.includes('city') || item.order_id ? normalizeCityTrip(item) : null; } catch { return null; } }
...
export async function listIntercityOffers(): Promise<IntercityOffer[]> { const data = await request<any>('/intercity/offers'); return unwrapItems<BackendIntercityOffer>(data).map((item) => normalizeIntercityOffer(item)); }
...
export async function getCurrentIntercityTrip(): Promise<IntercityTrip | null> { try { const item = await getCurrentTrip(); if (!item) return null; const type = String(item.trip_type || item.source_type || ''); return type.includes('intercity') || item.source_type ? normalizeIntercityTrip(item) : null; } catch { return null; } }
export async function updateIntercityTripStatus(tripId: number, status: string): Promise<IntercityTrip> { const data = await requestFirst<any>([`/intercity/trips/${tripId}/status`, `/trips/intercity/${tripId}/status`], { method: 'POST', body: JSON.stringify({ status }) }); return normalizeIntercityTrip(unwrapItem(data)); }

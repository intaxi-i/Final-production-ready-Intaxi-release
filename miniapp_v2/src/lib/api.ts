...
type BackendVehicle = { brand?: string | null; model?: string | null; plate?: string | null; color?: string | null; capacity?: string | number | null; vehicle_class?: string | null };
type BackendUser = { id?: number | null; tg_id?: number | null; full_name?: string | null; username?: string | null; language?: string | null; country?: string | null; country_code?: string | null; city?: string | null; active_role?: UserRole | null; is_verified?: boolean | null; is_blocked?: boolean | null; rating?: number | null; rating_count?: number | null; photo_url?: string | null; vehicle?: BackendVehicle | null };
...
function unwrapUser(data: ApiEnvelope<BackendUser>): BackendUser {
  if (data && typeof data === 'object' && 'user' in data && data.user) return data.user;
  return data as BackendUser;
}

function normalizeMe(rawEnvelope: ApiEnvelope<BackendUser>): UserMe & { is_verified?: boolean; vehicle?: BackendVehicle | null } {
  const raw = unwrapUser(rawEnvelope);
  ...
  return {
    ...
    city_id: raw?.city ? Number(raw.city) || null : null,
...
function normalizeIntercityTrip(raw: any): IntercityTrip { return { id: Number(raw?.id ?? raw?.trip_id ?? 0), mode: raw?.mode || 'regular', source_type: raw?.source_type || raw?.trip_type || raw?.kind || 'request', source_id: Number(raw?.source_id ?? raw?.id ?? 0), passenger_user_id: Number(raw?.passenger_user_id ?? raw?.passenger_tg_id ?? 0), driver_user_id: Number(raw?.driver_user_id ?? raw?.driver_tg_id ?? 0), vehicle_id: raw?.vehicle_id ?? null, final_price: Number(raw?.final_price ?? raw?.price ?? raw?.price_offer ?? 0), currency: raw?.currency || countryCurrency(raw?.country), status: raw?.status || 'accepted' }; }

function unwrapItems<T>(data: T[] | { items?: T[] }): T[] { return Array.isArray(data) ? data : data?.items || []; }
function unwrapItem<T>(data: T | { item?: T } | { order?: T } | { trip?: T }): T { if (data && typeof data === 'object' && 'item' in data && data.item) return data.item; if (data && typeof data === 'object' && 'order' in data && data.order) return data.order; if (data && typeof data === 'object' && 'trip' in data && data.trip) return data.trip; return data as T; }
function currentTripItem(data: any) { return data?.item || data || null; }
function currentTripType(item: any) { return String(item?.trip_type || item?.source_type || item?.kind || '').toLowerCase(); }
function isCurrentCityTrip(item: any) { const type = currentTripType(item); if (type.startsWith('intercity')) return false; if (type === 'city_trip') return true; return Boolean(item?.order_id && (item?.from_address || item?.pickup_address || item?.to_address || item?.destination_address)); }
function isCurrentIntercityTrip(item: any) { const type = currentTripType(item); return type.startsWith('intercity') || type === 'route' || type === 'request'; }
...
export async function getMe(): Promise<UserMe & { is_verified?: boolean; vehicle?: BackendVehicle | null }> { return normalizeMe(await request<ApiEnvelope<BackendUser>>('/me')); }
export async function updateMe(input: UserProfileInput): Promise<UserMe> { const data = await request<ApiEnvelope<BackendUser>>('/me/profile', { method: 'POST', body: JSON.stringify({ language: input.language, country: input.country_code, city: input.city_id ? String(input.city_id) : undefined }) }); return normalizeMe(data); }
export async function updateRole(activeRole: UserRole): Promise<UserMe> { const data = await request<ApiEnvelope<BackendUser>>('/me/role', { method: 'POST', body: JSON.stringify({ active_role: activeRole }) }); return normalizeMe(data); }
...
export async function listMyCityOrders(): Promise<CityOrder[]> { const data = await requestFirst<any>(['/city/my-orders', '/city/orders/my']); return unwrapItems<BackendCityOrder>(data).map((item) => normalizeCityOrder(item)); }

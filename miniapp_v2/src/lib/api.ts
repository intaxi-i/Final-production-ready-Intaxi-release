...
export async function listMyCityOrders(): Promise<CityOrder[]> { const data = await request<any>('/city/orders/my'); return unwrapItems<BackendCityOrder>(data).map((item) => normalizeCityOrder(item)).filter((item) => LIVE_ORDER_STATUSES.has(item.status)); }
...
export async function listIntercityOffers(): Promise<IntercityOffer[]> { const data = await request<any>('/intercity/offers'); return unwrapItems<BackendIntercityOffer>(data).map((item) => normalizeIntercityOffer(item)).filter((item) => item.status === 'active' || item.status === 'search'); }

...
def _trip_status_kb(trip_id: int, status: str, *, can_control: bool = False):
    ...
    builder.button(text="📱 Открыть поездку", url=_current_trip_link(trip_id))
    if can_control:
...
async def _notify_trip_status(bot: Bot, trip: CityTripV1, action: str):
    if action == 'onway':
        ...
        passenger_text = "✅ Поездка завершена.\n\nСпасибо, что воспользовались Intaxi."
        driver_text = "✅ Поездка завершена."
    try:
...
async def hotfix_city_create_start(message: types.Message, state: FSMContext):
    ...
    if user.is_verified and (user.active_role or 'driver') != 'passenger':
        await state.clear()
        await message.answer('Водитель не создаёт городской заказ. Переключите роль на пассажира в профиле.', reply_markup=home_webapp_menu(lang, is_driver_mode=True))
        return
    await state.clear()
...
async def hotfix_driver_accept(callback: types.CallbackQuery, bot: Bot):
    ...
    if not trip:
        await callback.answer('Не удалось принять заказ. Проверьте, что вы онлайн и без активной поездки.', show_alert=True)
        return
...
async def hotfix_driver_offer_price(callback: types.CallbackQuery, state: FSMContext):
    ...
    async with async_session() as session:
        ...
        if not driver or not order or not await _driver_can_receive_city_order(session, driver, order):
            await callback.answer('Заказ недоступен для предложения цены', show_alert=True)
            return
...
async def hotfix_trip_status(callback: types.CallbackQuery, bot: Bot):
    ...
    async with async_session() as session:
        ...
        if callback.from_user.id != trip.driver_tg_id:
            await callback.answer('Статус поездки меняет только водитель', show_alert=True)
            return
        if not next_status or (next_status != trip.status and next_status not in CITY_STATUS_NEXT.get(trip.status, set())):
            await callback.answer('Неверный переход статуса', show_alert=True)
            return

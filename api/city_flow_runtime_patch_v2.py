...
async def city_offer_detail(order_id: int | None = None, id: int | None = None, current_user: User = Depends(_current_user)) -> CityOrderEnvelope:
    ...
    async with async_session() as session:
        order = await session.scalar(select(CityOrderV1).where(CityOrderV1.id == target_id).with_for_update())
        if not order:
...
async def current_trip(original_endpoint: Callable | None = None, current_user: User = Depends(_current_user)) -> CurrentTripResponse:
    async with async_session() as session:
        ...
        order = await session.scalar(select(CityOrderV1).where(CityOrderV1.creator_tg_id == current_user.tg_id, CityOrderV1.status == "active").order_by(CityOrderV1.id.desc()))
        if order:
            return CurrentTripResponse(item=(await _order_schema(session, order, current_user)).model_dump())

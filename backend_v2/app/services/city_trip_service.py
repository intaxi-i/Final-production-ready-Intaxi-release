...
class CityTripService:
    ...
    async def accept_order(
        ...
        *,
        ...
        order = await session.scalar(select(CityOrder).where(CityOrder.id == order_id).with_for_update())
        if not order:
    ...
    async def accept_counteroffer(
        ...
        *,
        ...
        offer = await session.scalar(select(CityCounteroffer).where(CityCounteroffer.id == offer_id).with_for_update())
        if not offer:
    ...
    async def update_status(
        ...
        *,
        ...
        trip = await session.scalar(select(CityTrip).where(CityTrip.id == trip_id).with_for_update())
        if not trip:

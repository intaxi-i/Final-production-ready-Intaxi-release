...
class IntercityService:
    ...
    async def accept_request(self, session: AsyncSession, *, request_id: int, driver: User) -> IntercityTrip:
        req = await session.scalar(select(IntercityRequest).where(IntercityRequest.id == request_id).with_for_update())
        if not req:
    ...
    async def accept_route(self, session: AsyncSession, *, route_id: int, passenger: User) -> IntercityTrip:
        route = await session.scalar(select(IntercityRoute).where(IntercityRoute.id == route_id).with_for_update())
        if not route:

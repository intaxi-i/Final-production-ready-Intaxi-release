...
class WalletService:
    ...
    async def approve_topup(self, session: AsyncSession, *, topup_id: int, admin: User) -> PaymentTopupRequest:
        row = await session.scalar(select(PaymentTopupRequest).where(PaymentTopupRequest.id == topup_id).with_for_update())
        if not row:
        ...
        driver = await session.scalar(select(User).where(User.id == row.driver_user_id).with_for_update())
        if not driver:
